...
export const AddNewProject = ({ isAddNewProjectClicked, setIsAddNewProjectClicked, onProjectCreated }) => {
  ...
  const createProject = async (e) => {
    ...
    const asyncToastId = toast.loading("Creating project...")
    try {
      ...
      await axios.post(`${serverUrl}/api/v1/projects`, formData, {
        headers: {
      ...
      toast.update(asyncToastId, {
        render: `${errMessage}: Project creation failed`,
        type: "error",
        isLoading: false,
        autoClose: 5000,
      })
  ...
  return (
    <div
      className={`fixed bottom-0 inset-x-0 px-4 pb-4 sm:inset-0 flex items-center justify-center overflow-y-auto h-[100vh] z-[3000] transition-transform duration-300 ${
        isAddNewProjectClicked ? "translate-y-0" : "translate-y-[100%]"
      }`}
    >
      ...
      <div className="bg-white rounded-lg px-4 pt-5 pb-4 overflow-y-auto w-[99%] h-[98%] shadow-xl transform transition-all sm:max-w-3xl sm:w-full relative z-10">
        <h2 className="text-xl sm:text-2xl font-bold text-center mb-2">Create a New Project</h2>
        <AiOutlineClose
          className="w-8 h-8 fill-[#a68b6a] absolute right-5 cursor-pointer top-5 hover:opacity-70"
          onClick={() => setIsAddNewProjectClicked(false)}
        />
        <form action="" className="pt-6" onSubmit={createProject}>
          ...
          <div className="mb-4">
            ...
            <textarea
              ...
              placeholder="Describe the project, furniture used, design approach..."
            />
          </div>
          ...
          <div className="mb-6 relative">
            <label className="block font-medium mb-2">Project Images (1-4 images) *</label>
            <input
              type="file"
              ref={imgRef}
            ...
            <p className="text-sm text-gray-500 mt-1">
              {isUploading
                ...
                : uploadedImages.length >= 4
                  ...
                  : `Upload up to ${4 - uploadedImages.length} more image(s). First image will be the main display image.`}
            </p>
          </div>
