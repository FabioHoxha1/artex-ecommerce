...
export const DeleteProductModal = ({ isDeleteModalOn, setIsDeleteModalOn, _id }) => {
  ...
  // Only mount the modal into the DOM when it's visible or when a delete request is in progress.
  // This prevents rendering many hidden modal instances (one per table row) which can cause
  // layout thrashing and flicker in the product management table.
  if (!isDeleteModalOn && !isDeleteLoading) return null;
